...
# Function to download YouTube video using yt-dlp and local ffmpeg
def download_video(youtube_url, output_dir="videos", cookies_path=None):
    os.makedirs(output_dir, exist_ok=True)

    # Path to local ffmpeg binary
    ffmpeg_path = os.path.join(os.getcwd(), 'ffmpeg', 'ffmpeg')  # Adjust path as needed
    
    # Options for yt-dlp
    ydl_opts = {
        'format': 'best',  # Download best video and audio combined
        'outtmpl': os.path.join(output_dir, '%(id)s.%(ext)s'),  # Template for saving the file
        'ffmpeg_location': ffmpeg_path,  # Point yt-dlp to the local ffmpeg binary
